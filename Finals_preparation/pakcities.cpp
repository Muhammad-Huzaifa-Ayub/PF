...
int main()
{
	...
	if(db.is_open())
	{
		...
		while(db.good())
		{
			...
			db.getline(cityname, 21);  // we may use db.get(cityname, 21) instead without any surety
            // but we may use db.read(cityname, 20), cityname[20] = '\0' instead with the surety
			//db.clear(); // in 21 length stream no \n will put stream in error state
	...
	for(int j=0;j<CITY_COUNT;j++)
	{
		...
		cout << " ";
		cout << setw(3) << pakcities[j].name.length();
		cout << " ";
